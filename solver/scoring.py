...
def score_placement(
    ...
    # 3. Fragility: hard block — never place heavy (>2kg) on fragile boxes
    #    Each violation costs 0.005 final_score but placement only adds ~0.003 coverage
    fragility_score = 1.0
    if weight_kg > 2.0 and z > 0:
        fragile_below = state.get_fragile_boxes_at_top(z, x, y, x2, y2)
        if fragile_below:
            return -1.0  # Hard block: refuse this placement
